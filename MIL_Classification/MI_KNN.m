function [ output ] = MI_KNN( trainBags, trainLabel, testBags, k_of_kNN )

num_of_class = length(unique( trainLabel ));
num_of_train = length( trainBags );
num_of_test = length( testBags );
value_1 = 1;
value_2 = 1;
value_3 = 1;
value_4 = 1;
...
% value_5 = 1/sum(trainLabel==-1);
    ...
    for i = 1:num_of_test
      for j = 1:num_of_train
          distanceMatrix(j,i) = maxHausdorff( testBags{i}, trainBags{j} );
      end
    end
